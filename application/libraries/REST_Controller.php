...
trait REST_Controller {
	...
	protected function _clear_api_key(){
		// Get the api key name variable set in the rest config file
        $api_key_variable = $this->config->item('rest_key_name');
		$api_key_lifetime = $this->config->item('rest_key_lifetime');
		//var_dump($api_key_variable);
        ...
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name)))
        {
			$SQL = "
				UPDATE `keys`
				SET status = 'N'
				WHERE updated_at < SUBDATE(CURRENT_TIMESTAMP, INTERVAL {$api_key_lifetime} MINUTE)
				AND status = 'Y'
			";
    ...
    protected function _detect_api_key()
    {
        ...
        if (($key = isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server($key_name)))
        {
            if ( ! ($row = $this->rest->db->where($this->config->item('rest_key_column'), $key)->where('status', 'Y')->get($this->config->item('rest_keys_table'))->row()))
            {
                return FALSE;
            }
    ...
    {
		//exdebug($this->router->class);
		//exdebug($this->router->method);
        // Assign the class/method auth type override array from the config
        $auth_override_class_method = $this->config->item('auth_override_class_method');
		//exdebug($auth_override_class_method);
    ...
    {
		$this->_put_args = $this->fetchContent();
        if ($key === NULL)
        {
    ...
    {
		/*
		exdebug(file_get_contents('php://input'));
		parse_str(file_get_contents('php://input'), $_delete);
		exdebug($_REQUEST);
		*/
		$this->_delete_args = $this->fetchContent();
		//exdebug($this->_delete_args);
        if ($key === NULL)
        {
    ...
    {
		$this->_patch_args = $this->fetchContent();
        if ($key === NULL)
        {
